...
T_1=288;P_1=101.35;n_c=0.87;n_GT=0.89;n_PT=0.89;m_dot=0.453; %variable
for T_3=1400:200:2000
    ...
    for PR=10:1:60
        ...
        w_PTs(i)=(H_4a(i)-H_5s(i))/(X(i)*28.965);
        w_PTa(i)=w_PTs(i)*n_PT;
        w_PTabar(i)=w_PTa(i)*28.965*X(i);
        H_5a(i)=H_4a(i)-w_PTabar(i);
        a=300;b=2500;
        tol=20;
        while abs(tol)>0.001
            m=(a+b)/2;
            hCo2_m=interp1(TCo2,hCo2,m,'spline');hH2O_m=interp1(TH2O,hH2O,m,'spline');hO2_m=interp1(TO2,hO2,m,'spline');h_m=interp1(T,h,m,'spline');
            eq=8*hCo2_m+9*hH2O_m+X(i)*28.965*h_m-12.5*hO2_m-H_5a(i);
            hCo2_a=interp1(TCo2,hCo2,a,'spline');hH2O_a=interp1(TH2O,hH2O,a,'spline');hO2_a=interp1(TO2,hO2,a,'spline');h_a=interp1(T,h,a,'spline');
            eqa=8*hCo2_a+9*hH2O_a+X(i)*28.965*h_a-12.5*hO2_a-H_5a(i);
            hCo2_b=interp1(TCo2,hCo2,b,'spline');hH2O_b=interp1(TH2O,hH2O,b,'spline');hO2_b=interp1(TO2,hO2,b,'spline');h_b=interp1(T,h,b,'spline');
            eqb=8*hCo2_b+9*hH2O_b+X(i)*28.965*h_b-12.5*hO2_b-H_5a(i);
            if eq*eqa<0
                b=m;
            elseif eq*eqb<0
                a=m;
        ...
        T_5a(i)=m;
        q_indot(i)=q_in(i)*m_dot;
        m_fdot(i)=m_dot*f(i)
        w_PTadot(i)=w_PTa(i)*m_dot
        n_th(i)=w_PTadot(i)*100/q_indot(i);
        SFC(i)=m_fdot(i)*3600/w_PTadot(i);
        HR(i)=q_indot(i)*3600/w_PTadot(i);
    end
    PR=10:1:60;
    figure(1);
    subplot(2,2,1);
