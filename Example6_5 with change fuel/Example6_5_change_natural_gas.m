...
for T_3=1900%1400:200:2000
    ...
    for i=1:1:60
        ...
        syms x;
        eqn=1*hCo2_3+2*hH2O_3+x*h_3*28.965-2*hO2_3==1*(-74809)+x*h_2a(i)*28.965;
        x=solve(eqn,x);
        X(i)=double(x); %find dry air's mole
        excess_air(i)=(X(i)-59.67)/59.67*100; % %excess air
        f(i)=16.043/(X(i)*28.965); %fuel air ratio
        q_in(i)=50012*f(i);
        w_GTabar(i)=(w_ca(i)+alpha*w_fa)*28.965*X(i);
        w_GTsbar(i)=w_GTabar(i)/n_GT;
        H_3(i)=1*hCo2_3+2*hH2O_3+X(i)*28.965*h_3-2*hO2_3;
        ...
        while abs(tol)>0.001
            ...
            eq=1*hCo2_m+2*hH2O_m+X(i)*28.965*h_m-2*hO2_m-H_4s(i);
            ...
            eqa=1*hCo2_a+2*hH2O_a+X(i)*28.965*h_a-2*hO2_a-H_4s(i);
            ...
            eqb=1*hCo2_b+2*hH2O_b+X(i)*28.965*h_b-2*hO2_b-H_4s(i);
            if eq*eqa<0
                b=m;
            elseif eq*eqb<0
                a=m;
        ...
        syms x;
        eqn=1*sCo2_4s(i)+2*sH2O_4s(i)+X(i)*28.965*s_4s(i)-2*sO2_4s(i)-(1*sCo2_3+2*sH2O_3+X(i)*28.965*s_3-2*sO2_3)==8.314*(1+2+X(i)-2)*log(x/P_3(i));
        x=solve(eqn,x);
        P_4(i)=double(x);
        ...
        while abs(tol)>0.001
            m=(a+b)/2;
            hCo2_m=interp1(TCo2,hCo2,m,'spline');hH2O_m=interp1(TH2O,hH2O,m,'spline');hO2_m=interp1(TO2,hO2,m,'spline');h_m=interp1(T,h,m,'spline');
            eq=1*hCo2_m+2*hH2O_m+X(i)*28.965*h_m-2*hO2_m-H_4a(i);
            hCo2_a=interp1(TCo2,hCo2,a,'spline');hH2O_a=interp1(TH2O,hH2O,a,'spline');hO2_a=interp1(TO2,hO2,a,'spline');h_a=interp1(T,h,a,'spline');
            eqa=1*hCo2_a+2*hH2O_a+X(i)*28.965*h_a-2*hO2_a-H_4a(i);
            hCo2_b=interp1(TCo2,hCo2,b,'spline');hH2O_b=interp1(TH2O,hH2O,b,'spline');hO2_b=interp1(TO2,hO2,b,'spline');h_b=interp1(T,h,b,'spline');
            eqb=1*hCo2_b+2*hH2O_b+X(i)*28.965*h_b-2*hO2_b-H_4a(i);
            if eq*eqa<0
                b=m;
            elseif eq*eqb<0
                a=m;
        ...
        while abs(tol)>0.001
            ...
            eq=1*sCo2_m+2*sH2O_m+X(i)*28.965*s_m-2*sO2_m-(1*sCo2_4a(i)+2*sH2O_4a(i)+X(i)*28.965*s_4a(i)-2*sO2_4a(i))-8.314*(1+2+X(i)-2)*log(P_5/P_4(i));
            ...
            eqa=1*sCo2_a+2*sH2O_a+X(i)*28.965*s_a-2*sO2_a-(1*sCo2_4a(i)+2*sH2O_4a(i)+X(i)*28.965*s_4a(i)-2*sO2_4a(i))-8.314*(1+2+X(i)-2)*log(P_5/P_4(i));
            ...
            eqb=1*sCo2_b+2*sH2O_b+X(i)*28.965*s_b-2*sO2_b-(1*sCo2_4a(i)+2*sH2O_4a(i)+X(i)*28.965*s_4a(i)-2*sO2_4a(i))-8.314*(1+2+X(i)-2)*log(P_5/P_4(i));
            if eq*eqa<0
                b=m;
            elseif eq*eqb<0
                a=m;
        ...
        H_5s(i)=1*hCo2_5s(i)+2*hH2O_5s(i)+X(i)*28.965*h_5s(i)-2*hO2_5s(i);
    ...
    hold on;
    subplot(2,2,2);
    plot(PR,thrust,'r','LineWidth',1.5);
    xlabel('PR','Fontsize',18);
    ...
    hold on;
    subplot(2,2,[3,4]);
    plot(PR,thrust_frome_core,'r','LineWidth',1.5);
    xlabel('PR','Fontsize',18);
...
sgtitle('Real Combustion with Change Fuel','Fontsize',24);