...
T_1=288;P_1=101.35;n_c=0.88;n_GT=0.91;n_PT=0.90;m_cdot=1;PRF=1.35;n_fc=0.87;n_fn=1;alpha=10; %variable
...
for T_3=1900%1600:200:2000
    PR=10:1:70;
    PR(18)=[];
    for i=1:1:60
        P_2(i)=PR(i)*P_1;P_3(i)=PR(i)*P_1;
        ...
        syms x;
        eqn=4*hCo2_3+5*hH2O_3+x*h_3*28.965-6.5*hO2_3==1*(-147655)+x*h_2a(i)*28.965;
        x=solve(eqn,x);
        X(i)=double(x); %find dry air's mole
        excess_air(i)=(X(i)-59.67)/59.67*100; % %excess air
        f(i)=58.1248/(X(i)*28.965); %fuel air ratio
        q_in(i)=45342*f(i);
        w_GTabar(i)=(w_ca(i)+alpha*w_fa)*28.965*X(i);
        w_GTsbar(i)=w_GTabar(i)/n_GT;
        H_3(i)=4*hCo2_3+5*hH2O_3+X(i)*28.965*h_3-6.5*hO2_3;
        ...
        while abs(tol)>0.001
            ...
            eq=4*hCo2_m+5*hH2O_m+X(i)*28.965*h_m-6.5*hO2_m-H_4s(i);
            ...
            eqa=4*hCo2_a+5*hH2O_a+X(i)*28.965*h_a-6.5*hO2_a-H_4s(i);
            ...
            eqb=4*hCo2_b+5*hH2O_b+X(i)*28.965*h_b-6.5*hO2_b-H_4s(i);
            if eq*eqa<0
                b=m;
            elseif eq*eqb<0
                a=m;
        ...
        syms x;
        eqn=4*sCo2_4s(i)+5*sH2O_4s(i)+X(i)*28.965*s_4s(i)-6.5*sO2_4s(i)-(4*sCo2_3+5*sH2O_3+X(i)*28.965*s_3-6.5*sO2_3)==8.314*(4+5+X(i)-6.5)*log(x/P_3(i));
        x=solve(eqn,x);
        P_4(i)=double(x);
        ...
        while abs(tol)>0.001
            m=(a+b)/2;
            hCo2_m=interp1(TCo2,hCo2,m,'spline');hH2O_m=interp1(TH2O,hH2O,m,'spline');hO2_m=interp1(TO2,hO2,m,'spline');h_m=interp1(T,h,m,'spline');
            eq=4*hCo2_m+5*hH2O_m+X(i)*28.965*h_m-6.5*hO2_m-H_4a(i);
            hCo2_a=interp1(TCo2,hCo2,a,'spline');hH2O_a=interp1(TH2O,hH2O,a,'spline');hO2_a=interp1(TO2,hO2,a,'spline');h_a=interp1(T,h,a,'spline');
            eqa=4*hCo2_a+5*hH2O_a+X(i)*28.965*h_a-6.5*hO2_a-H_4a(i);
            hCo2_b=interp1(TCo2,hCo2,b,'spline');hH2O_b=interp1(TH2O,hH2O,b,'spline');hO2_b=interp1(TO2,hO2,b,'spline');h_b=interp1(T,h,b,'spline');
            eqb=4*hCo2_b+5*hH2O_b+X(i)*28.965*h_b-6.5*hO2_b-H_4a(i);
            if eq*eqa<0
                b=m;
            elseif eq*eqb<0
                a=m;
        ...
        while abs(tol)>0.001
            ...
            eq=4*sCo2_m+5*sH2O_m+X(i)*28.965*s_m-6.5*sO2_m-(4*sCo2_4a(i)+5*sH2O_4a(i)+X(i)*28.965*s_4a(i)-6.5*sO2_4a(i))-8.314*(4+5+X(i)-6.5)*log(P_5/P_4(i));
            ...
            eqa=4*sCo2_a+5*sH2O_a+X(i)*28.965*s_a-6.5*sO2_a-(4*sCo2_4a(i)+5*sH2O_4a(i)+X(i)*28.965*s_4a(i)-6.5*sO2_4a(i))-8.314*(4+5+X(i)-6.5)*log(P_5/P_4(i));
            ...
            eqb=4*sCo2_b+5*sH2O_b+X(i)*28.965*s_b-6.5*sO2_b-(4*sCo2_4a(i)+5*sH2O_4a(i)+X(i)*28.965*s_4a(i)-6.5*sO2_4a(i))-8.314*(4+5+X(i)-6.5)*log(P_5/P_4(i));
            if eq*eqa<0
                b=m;
            elseif eq*eqb<0
                a=m;
        ...
        H_5s(i)=4*hCo2_5s(i)+5*hH2O_5s(i)+X(i)*28.965*h_5s(i)-6.5*hO2_5s(i);
    ...
    figure('color','white');
    subplot(2,2,1);
    plot(PR,TSFC,'g','LineWidth',1.5);
    xlabel('PR','Fontsize',18);
    ylabel('TSFC(mg/N*s)','Fontsize',18);
    newcolors={'r','g','b','k'};
    colororder(newcolors);
    labels={'C_4H_1_0'};
    legend(labels,'Location','northeast','Fontsize',18);
    set(gca,'Fontname','Times New Roman','FontWeight','bold');
    hold on;
    subplot(2,2,2);
    plot(PR,thrust,'g','LineWidth',1.5);
    xlabel('PR','Fontsize',18);
    ...
    hold on;
    subplot(2,2,[3,4]);
    plot(PR,thrust_frome_core,'g','LineWidth',1.5);
    xlabel('PR','Fontsize',18);
    ylabel('thrust from the core','Fontsize',18);
    newcolors={'r','g','b','k'};
    colororder(newcolors);
    labels={'C_4H_1_0'};
    legend(labels,'Location','northeast','Fontsize',18);
...
sgtitle('Real Combustion with Natural Gas vs. C8H18','Fontsize',24);